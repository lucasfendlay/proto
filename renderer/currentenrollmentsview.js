document.addEventListener('DOMContentLoaded', async function () {
    ...
    async function addHouseholdMemberToUI(member) {
        ...
        const residenceStatus = member.residenceStatus ? member.residenceStatus.toLowerCase() : 'other';
        ...
        if ((years >= 18 && isDisabled) || (years >= 50 && isWidowed) || (years >= 65 && residenceStatus !== 'other')) {
            try {
                const response = await fetch(`/get-client/${clientId}`);
                const client = await response.json();
    ...
    async function saveDefaultSelection(clientId, memberId, question, value) {
        try {
            ...
            if (window.eligibilityChecks) {
                ...
                console.warn('window.eligibilityChecks is not defined.');
            }
        } catch (error) {
...
window.saveDefaultSelection = saveDefaultSelection; // Expose the function globally